...
mod tests{
    ...
    fn it_sends_an_over_75_percent_warning_message() {
        ...
        let mut limit_tracker = LimitTracker::new(&mock_messenger,90);
        limit_tracker.set_value(80);
        assert_eq!(mock_messenger.borrow().sent_messages.len(),1);
    }
}

// borrow() -> Ref<T> 获得不可修改借用
// borrow_mut() -> RefMut<T> 获得可修改借用
// 只能借用一次
...
fn main(){
    ...
    // 解引用 RefCell<T> 使用可变借用borrow_mut()
    *value.borrow_mut() += 10;
    println!("a after = {a:?}");
    println!("b after = {b:?}");
    println!("c after = {c:?}");
